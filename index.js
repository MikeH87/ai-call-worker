...
console.log("index.js — v4.2.0-qualification");

// Log hubspot exports for diagnostics
try {
...
// ---------- routes ----------
app.get("/", (req, res) => {
  res.send("AI Call Worker v4.2.0 (Qualification + IC) ✅");
});
...
function idify(val) {
  ...
  if (typeof val === "object")
    return idify(val.id || val.hs_object_id || val.value || val.toString?.());
  const s = String(val).trim();
  return s.length ? s : "";
}
...
function extractFromWebhook(body = {}) {
  ...
  const props =
    b.properties || b.objectProperties || b.object?.properties || {};
...
app.post("/process-call", async (req, res) => {
  try {
    ...
    if (!callId || !recordingUrl) {
      ...
      return res
        .status(400)
        .json({ ok: false, error: "callId and recordingUrl required" });
    }
    ...
    if (typeLabel === "Qualification Call") {
      ...
      const scorecardId = await createQualificationScorecard({
        callId,
        contactIds,
        data: analysis,
      });
    ...
    // -----------------
    // INITIAL CONSULTATION (existing path)
    // -----------------
